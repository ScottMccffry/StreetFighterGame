import React, { useContext } from 'react';
import WalletContext from '../../context/WalletContext';
import styles from './Header.module.css';


function Header() {
  const { isConnected, connectWallet } = useContext(WalletContext);

    return (
      <div className={`${styles.container} ${styles.containerMd}`}>
      <div className={`${styles.menuButton} ${styles.menuButtonMd}`}>
      <svg
  xmlns="http://www.w3.org/2000/svg"
  className={`${styles.menuIcon}`}  // added className
  fill="none"
  viewBox="0 0 24 24"
  stroke="currentColor"
>
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M4 6h16M4 12h16M4 18h16"
                />
        </svg>
      </div>
    
      <div className={`${styles.logoContainer} ${styles.logoContainerMd}`}>
        <div className={styles.logo}></div>
        <div className={styles.logoTitle}>METAFIGHT</div>
      </div>
      <form className={`${styles.searchForm} ${styles.searchFormMd}`}>
      <input
        type="text"
        className={styles.searchInput}
        placeholder="Search for collection"
      />
      <span className={styles.searchIcon}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className={styles.searchIconSvg}
          viewBox="0 0 20 20"
          fill="currentColor"
        >
          <path
                  fillRule="evenodd"
                  d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z"
                  clipRule="evenodd"
                />


</svg>
      </span>
    </form>
  
      {isConnected ? (
        <div className={styles.connectedUser}>
          <div className={styles.notificationIcon}>
          <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-7 w-7"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6zM10 18a3 3 0 01-3-3h6a3 3 0 01-3 3z" />
              </svg>
          </div>
          <div className={styles.notificationDot}>
            <div className={styles.notificationDotInner} />
          </div>
          <img
            src="https://assets.codepen.io/3685267/nft-dashboard-pro-1.jpg"
            alt="user"
            className={styles.avatar}
          />
          <div className={styles.dropdownIcon}>
          <img
              src="https://assets.codepen.io/3685267/nft-dashboard-pro-1.jpg"
              alt="user"
              className="w-10 h-10 rounded-full ml-4"
            />

            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-7 w-7 ml-4"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
                fillRule="evenodd"
                d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                clipRule="evenodd"
              />
            </svg>
          </div>
        </div>
      ) : (
        <div className={styles.button}>
          <button onClick={connectWallet} className={styles.connectButton}>
            Connect
          </button>
        </div>
      )}
  
   
  </div>

    );
  }
  export default Header;